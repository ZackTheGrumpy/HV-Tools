...
title HV Bypass Tool v4.0.3 - Modernized Edition

:: =====================================================================
:: STEP 1: Administrator Privileges Check & Safe Self-Elevation
:: =====================================================================
net session >nul 2>&1
if !errorLevel! == 0 goto :has_admin

echo [*] Requesting administrator privileges...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Start-Process -FilePath '%comspec%' -ArgumentList ('/c ""' +  + '"" %*') -Verb RunAs" >nul 2>&1
if !errorLevel! neq 0 (
    echo.
...
:has_admin
:: =====================================================================
:: STEP 2: Verify Windows PowerShell Availability
:: =====================================================================
...
:: =====================================================================
:: STEP 3: Launch Modernized PowerShell Application
:: =====================================================================
powershell -NoProfile -ExecutionPolicy Bypass -Command "$c = Get-Content -LiteralPath $env:SCRIPT_PATH -Encoding UTF8 | Select-Object -Skip 46 | Out-String; & ([scriptblock]::Create($c)) @args" %*
if !errorLevel! neq 0 (
    echo.
    echo [ERROR] Tool exited with an error.
    pause
)
exit /B
: end batch portion #>
...
# =========================================================================
# HV BYPASS TOOL v4.0.3 - ADVANCED MODERNIZED EDITION
# =========================================================================
# Clean native architecture, Widescreen Dashboard UI, 1-Click Auto Setup,
# Auto-Restart Integration, Collapsible Advanced Tools & Meltdown Fix
# =========================================================================
...
function Set-VBSAndHVCIStatus([bool]$Enable) {
    ...
    try {
        ...
        bcdedit /set hypervisorlaunchtype $btype 2>$null | Out-Null
        bcdedit /set "{current}" hypervisorlaunchtype $btype 2>$null | Out-Null
        bcdedit /set "{hypervisorsettings}" hypervisorlaunchtype $btype 2>$null | Out-Null
        bcdedit /set "{bootloadersettings}" hypervisorlaunchtype $btype 2>$null | Out-Null
        bcdedit /set vsmlaunchtype $btype 2>$null | Out-Null
        bcdedit /set "{current}" vsmlaunchtype $btype 2>$null | Out-Null
        
        if (-not $Enable) {
            # Disengage Windows Hello VBS & Enhanced Sign-in Security scenarios
            Disable-WindowsHelloProtection -Silent
            # Intel KVA Shadow Meltdown Override (required for older/mid Intel CPUs)
            Set-ItemProperty -Path $memPath -Name "FeatureSettingsOverride" -Value 2 -Type DWord -Force -ErrorAction SilentlyContinue | Out-Null
            Set-ItemProperty -Path $memPath -Name "FeatureSettingsOverrideMask" -Value 3 -Type DWord -Force -ErrorAction SilentlyContinue | Out-Null
        } else {
            Remove-ItemProperty -Path $memPath -Name "FeatureSettingsOverride" -ErrorAction SilentlyContinue | Out-Null
            Remove-ItemProperty -Path $memPath -Name "FeatureSettingsOverrideMask" -ErrorAction SilentlyContinue | Out-Null
        }
...
function Restart-DirectToUEFI {
    ...
    $confirm = [System.Windows.Forms.MessageBox]::Show(
        "Restart into BIOS / UEFI Setup now?`n`n" +
        "BIOS Key: $hint`n" +
        "In BIOS, turn Secure Boot [Disabled] and press F10.",
        "Reboot to BIOS / UEFI",
...
function Revert-ToSafeMode {
    ...
    if ($Mode -eq 'CLI' -or $Script:NextMode -eq 'CLI') {
        Write-Host ""
        Write-Host "================================================================" -ForegroundColor Yellow
        Write-Host "                    REVERT TO SAFE MODE" -ForegroundColor Yellow
        Write-Host "================================================================" -ForegroundColor Yellow
        Write-Host "  This restores factory default Windows security:" -ForegroundColor White
        Write-Host "  - Re-enables VBS and Memory Integrity (HVCI)" -ForegroundColor White
        Write-Host "  - Disables Test Signing (Required for Valorant / Faceit)" -ForegroundColor White
        Write-Host ""
        $c = Read-Host "Proceed with restoring factory defaults? (Y/N) [Default: N]"
        if ([string]::IsNullOrWhiteSpace($c) -or $c.Trim().ToUpper() -ne 'Y') {
            Write-Host "Operation cancelled." -ForegroundColor Yellow
            return
        }
    } else {
        $confirm = [System.Windows.Forms.MessageBox]::Show(
            "Restore factory default Windows security?`n`n" +
            "This re-enables VBS, HVCI, and turns OFF testing mode (required for Valorant / Faceit).`n`n" +
            "Proceed?",
            "Revert to Safe Mode",
            [System.Windows.Forms.MessageBoxButtons]::YesNo,
            [System.Windows.Forms.MessageBoxIcon]::Question
    ...
    if ($Mode -eq 'CLI' -or $Script:NextMode -eq 'CLI') {
        Write-Host "[OK] Default security restored!" -ForegroundColor Green
        $ask = Read-Host "Restart computer now? (Y/N) [Default: Y]"
        if ([string]::IsNullOrWhiteSpace($ask) -or $ask.Trim().ToUpper() -eq 'Y') {
            Write-Host "[*] Restarting computer in 5 seconds..." -ForegroundColor Yellow
            shutdown /r /t 5 -c "Restarting to restore standard Windows security..."
            Start-Sleep -Seconds 3
            $Script:NextMode = "Exit"
        }
    } else {
        $askReboot = [System.Windows.Forms.MessageBox]::Show(
            "Default security restored!`n`nRestart computer now?",
            "Restart Required",
            [System.Windows.Forms.MessageBoxButtons]::YesNo,
            [System.Windows.Forms.MessageBoxIcon]::Question
        )
        if ($askReboot -eq [System.Windows.Forms.DialogResult]::Yes) {
            shutdown /r /t 5 -c "Restarting to restore standard Windows security..."
        }
    }
}
...
function Add-DefenderExclusion {
    ...
    try {
        ...
        if ($Script:NextMode -eq 'CLI') {
            Write-Host "[OK] Defender exclusion added: $folder" -ForegroundColor Green
        } else {
        ...
        if ($Script:NextMode -eq 'CLI') {
            Write-Host "[!] Failed to add Defender exclusion: $($_.Exception.Message)" -ForegroundColor Yellow
        } else {
            [System.Windows.Forms.MessageBox]::Show(
...
function Confirm-WindowsHelloPreFlight([string]$CallingAction = "Disable VBS") {
    ...
    if ($st.HelloConflict) {
        ...
        $msg = "Windows Hello PIN protection is active.`n`n" +
               "Click [Yes] to auto-protect your login and proceed.`n" +
               "Click [No] to open Windows Settings and remove PIN first.`n" +
...
function Confirm-WindowsHelloPreFlightCli {
    ...
    if ($st.HelloConflict) {
        Write-Host ""
        Write-Host "================================================================" -ForegroundColor Yellow
        Write-Host "                    WINDOWS HELLO NOTICE" -ForegroundColor Yellow
        Write-Host "================================================================" -ForegroundColor Yellow
        Write-Host "  Windows Hello PIN is active with VBS protection." -ForegroundColor White
        Write-Host "----------------------------------------------------------------" -ForegroundColor DarkGray
        Write-Host "  [1] Auto-protect PIN login & proceed (Recommended)" -ForegroundColor Green
        Write-Host "  [2] Open Windows Settings to remove PIN first" -ForegroundColor Cyan
...
function Get-LiveStatuses {
    # 1. Virtualization Status (VT-x / AMD SVM in BIOS)
    $vtx = $false
    try {
        ...
        if ($proc -and $proc.VirtualizationFirmwareEnabled -eq $true) { $vtx = $true }
    } catch {}

    # Check if Windows Hyper-V is running in memory
    $hypPresent = $false
    try {
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -Property HypervisorPresent -ErrorAction SilentlyContinue
        if ($cs -and $cs.HypervisorPresent) { $hypPresent = $true }
    } catch {}
    ...
    # Ready requires: Virtualization ON in BIOS, VBS/HVCI OFF, and Windows Hyper-V NOT running
    $isReady = ($vtx -and -not $vbs -and -not $hvci -and -not $hypPresent)

    return [PSCustomObject]@{
        Virt            = $vtx
        HypActive       = $hypPresent
        SecureBoot      = $sb
        VBS             = $vbs
        HVCI            = $hvci
        IsReady         = $isReady
...
function Start-GuiMode {
    ...
    $Script:BtnAuto = New-StyledButton "[*] 1-CLICK EASY SETUP (RECOMMENDED)" 12 50 354 52 $Colors.BtnGreen {
        ...
        $confirm = [System.Windows.Forms.MessageBox]::Show(
            "1-CLICK EASY SETUP will:`n`n" +
            "1. Disable VBS, Memory Integrity & Hyper-V`n" +
            "2. Apply Intel CPU optimizations (Meltdown override)`n" +
            "3. Protect Windows Hello PIN login`n" +
            "4. Whitelist folder in Windows Defender`n" +
            "5. Enable Driver Test Signing`n`n" +
            "Setup runs silently in background (~10-15s).`n`nStart now?",
            "1-Click Easy Setup",
            [System.Windows.Forms.MessageBoxButtons]::YesNo,
            [System.Windows.Forms.MessageBoxIcon]::Question
        )
        ...
        $Script:StatusBar.Text = "Status: [WAIT] Easy Setup is running silently in background. Please wait ~15s..."
        $Script:StatusBar.ForeColor = $Colors.StatusWarn
...
# Deep BCD hypervisor release across all boot containers
bcdedit /set hypervisorlaunchtype off 2>$null | Out-Null
bcdedit /set "{current}" hypervisorlaunchtype off 2>$null | Out-Null
bcdedit /set "{hypervisorsettings}" hypervisorlaunchtype off 2>$null | Out-Null
bcdedit /set "{bootloadersettings}" hypervisorlaunchtype off 2>$null | Out-Null
bcdedit /set vsmlaunchtype off 2>$null | Out-Null
bcdedit /set "{current}" vsmlaunchtype off 2>$null | Out-Null

# Disengage Windows Hello VBS & ESS Scenarios
$whScenarios = @(
    "HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\WindowsHello",
    "HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\SecureBiometrics",
    "HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\WindowsHelloSecureBiometrics",
...
# Intel CPU Meltdown / KVA Shadow fix
Set-ItemProperty -Path $memPath -Name "FeatureSettingsOverride" -Value 2 -Type DWord -Force -ErrorAction SilentlyContinue | Out-Null
Set-ItemProperty -Path $memPath -Name "FeatureSettingsOverrideMask" -Value 3 -Type DWord -Force -ErrorAction SilentlyContinue | Out-Null

# Disable conflicting Windows features without reboot prompt during step
dism /online /Disable-Feature /FeatureName:Microsoft-Hyper-V-All /NoRestart 2>$null | Out-Null
dism /online /Disable-Feature /FeatureName:VirtualMachinePlatform /NoRestart 2>$null | Out-Null
dism /online /Disable-Feature /FeatureName:HypervisorPlatform /NoRestart 2>$null | Out-Null
dism /online /Disable-Feature /FeatureName:Containers-DisposableClientVM /NoRestart 2>$null | Out-Null

Add-MpPreference -ExclusionPath (Get-Location).Path -ErrorAction SilentlyContinue

bcdedit /set testsigning on 2>$null | Out-Null
bcdedit /set nointegritychecks on 2>$null | Out-Null
bcdedit /set "{current}" testsigning on 2>$null | Out-Null
bcdedit /set "{current}" nointegritychecks on 2>$null | Out-Null
'@
        ...
        $jobWatcher.add_Tick({
            try {
                if ($Script:BackgroundJob -and $Script:BackgroundJob.HasExited) {
                    ...
                    $askReboot = [System.Windows.Forms.MessageBox]::Show(
                        "1-Click Easy Setup Complete!`n`nA restart is required to apply the hypervisor changes.`n`nRestart computer now?",
                        "Setup Complete",
                        [System.Windows.Forms.MessageBoxButtons]::YesNo,
                        [System.Windows.Forms.MessageBoxIcon]::Question
                    )
                    if ($askReboot -eq [System.Windows.Forms.DialogResult]::Yes) {
                        shutdown /r /t 5 -c "Restarting to finalize Hypervisor and VBS configuration..."
                    }
                }
            } catch {
                $this.Stop()
    ...
    $manHeader.Location = New-Object System.Drawing.Point(12, 6)
    $manHeader.AutoSize = $true
    
    # 4 Main Primary Buttons (Always Visible)
    # Row 1: Tutorial Guide & CLI Mode (50/50 split)
    $btnTut = New-StyledButton "Tutorial Guide" 12 26 174 34 ([System.Drawing.Color]::FromArgb(35, 75, 120)) {
        Show-TutorialGuideDialog
    } -TooltipText "Open complete user tutorial and video guides"

    $btnCli = New-StyledButton "CLI Mode" 194 26 174 34 $Colors.BtnDark {
        $Script:NextMode = "CLI"
        $refreshTimer.Stop()
        $Script:MainForm.Close()
    } -TooltipText "Switch to lightweight text-based command line interface"

    # Row 2: Revert to Safe Mode
    $btnRevert = New-StyledButton "[[R]] Revert to Default (Safe Mode)" 12 64 356 34 ([System.Drawing.Color]::FromArgb(48, 38, 56)) {
        Revert-ToSafeMode
    } -TooltipText "Restore default factory Windows security for games like Valorant and Faceit"

    # Row 3: Exit Tool
    $btnExit = New-StyledButton "Exit Tool" 12 102 356 32 ([System.Drawing.Color]::FromArgb(34, 38, 48)) {
        $Script:NextMode = "Exit"
        $refreshTimer.Stop()
        $Script:MainForm.Close()
    } -TooltipText "Close the utility"

    # Advanced Collapsible Sub-Panel (Contains manual toggle buttons)
    $advSubPanel = New-Object System.Windows.Forms.Panel
    $advSubPanel.Location = New-Object System.Drawing.Point(12, 172)
    $advSubPanel.Size = New-Object System.Drawing.Size(356, 128)
    $advSubPanel.BackColor = [System.Drawing.Color]::FromArgb(20, 24, 34)
    $advSubPanel.BorderStyle = [System.Windows.Forms.BorderStyle]::FixedSingle
    $advSubPanel.Visible = $false

    # Manual Advanced Buttons inside Sub-Panel
    $Script:BtnTest = New-StyledButton "Toggle Testing Mode (BCD)" 6 4 342 27 $Colors.BtnDark {
        $st = Get-LiveStatuses
        if ($st.TestOn) {
            Set-TestingModeStatus -Enable $false | Out-Null
            if ($Script:UpdateUI) { & $Script:UpdateUI }
            [System.Windows.Forms.MessageBox]::Show("Testing Mode turned OFF in BCD.`nRestart to apply.", "Updated", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } else {
            Set-TestingModeStatus -Enable $true | Out-Null
            if ($Script:UpdateUI) { & $Script:UpdateUI }
            [System.Windows.Forms.MessageBox]::Show("Testing Mode turned ON in BCD.`nRestart to apply.", "Updated", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        }
    } -TooltipText "Toggle Windows Test Signing mode"

    $Script:BtnVbs = New-StyledButton "Toggle VBS & Memory Integrity" 6 34 342 27 $Colors.BtnDark {
        $st = Get-LiveStatuses
        if ($st.VBS -or $st.HVCI) {
            if (-not (Confirm-WindowsHelloPreFlight "Disable VBS")) { return }
            Set-VBSAndHVCIStatus -Enable $false | Out-Null
            if ($Script:UpdateUI) { & $Script:UpdateUI }
            [System.Windows.Forms.MessageBox]::Show("VBS & Memory Integrity DISABLED.`nRestart to commit.", "Updated", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } else {
            Set-VBSAndHVCIStatus -Enable $true | Out-Null
            if ($Script:UpdateUI) { & $Script:UpdateUI }
            [System.Windows.Forms.MessageBox]::Show("VBS & Memory Integrity RE-ENABLED.`nRestart to commit.", "Updated", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        }
    } -TooltipText "Toggle Virtualization-Based Security"

    $btnUefi = New-StyledButton "Reboot Directly to BIOS / UEFI" 6 64 342 27 ([System.Drawing.Color]::FromArgb(45, 60, 85)) {
        Restart-DirectToUEFI
    } -TooltipText "Restarts the PC straight into UEFI firmware settings"

    $Script:BtnSuo = New-StyledButton "Steam Unlock (SUO Setup)" 6 94 342 27 $Colors.BtnDark {
        Start-SuoDownloaderWorkflow
    } -TooltipText "Download and launch official Steam Unlock installer"

    $advSubPanel.Controls.AddRange(@($Script:BtnTest, $Script:BtnVbs, $btnUefi, $Script:BtnSuo))

    # Collapse / Expand Accordion Toggle Button
    $btnToggleAdv = New-StyledButton "[+] More Advanced Options (Manual Toggles)" 12 138 356 28 $Colors.BtnDark {
        if ($advSubPanel.Visible) {
            $advSubPanel.Visible = $false
            $this.Text = "[+] More Advanced Options (Manual Toggles)"
            $this.BackColor = $Colors.BtnDark
        } else {
            $advSubPanel.Visible = $true
            $this.Text = "[-] Hide Advanced Options"
            $this.BackColor = [System.Drawing.Color]::FromArgb(40, 50, 75)
        }
    } -TooltipText "Click to expand or collapse manual toggle controls"

    $manPanel.Controls.AddRange(@($manHeader, $btnTut, $btnCli, $btnRevert, $btnExit, $btnToggleAdv, $advSubPanel))
    ...
    $Script:UpdateUI = {
        ...
        if ($Script:ReadyBadge) {
            if ($st.IsReady) {
                ...
                $Script:ReadyBadge.Text = "[$([char]0x25B2) NOT READY FOR HYPERVISOR]"
        ...
        if ($Script:GuideBox) {
            if (-not $st.Virt) {
                $hint = Get-BiosKeyHint
                $Script:GuideBox.Text = "ACTION: Enable Virtualization in BIOS ($hint)"
                $Script:GuideBox.ForeColor = $Colors.StatusWarn
                $Script:GuideBox.BackColor = [System.Drawing.Color]::FromArgb(40, 50, 20)
            } elseif ($st.HypActive) {
                $Script:GuideBox.Text = "REBOOT REQUIRED: Windows Hyper-V is still active in memory"
                $Script:GuideBox.ForeColor = $Colors.StatusWarn
                $Script:GuideBox.BackColor = [System.Drawing.Color]::FromArgb(45, 35, 20)
                ...
                $Script:GuideBox.Text = "ALL SET: System is ready for Hypervisor!"
    ...
    while ($true) {
        ...
        switch ($choice.Trim().ToUpper()) {
            "A" {
                ...
                Write-Host "[*] Running 1-Click Easy Setup..." -ForegroundColor Cyan
                Suspend-BitLockerSafety
                Set-VBSAndHVCIStatus -Enable $false | Out-Null
                dism /online /Disable-Feature /FeatureName:Microsoft-Hyper-V-All /NoRestart 2>$null | Out-Null
                dism /online /Disable-Feature /FeatureName:VirtualMachinePlatform /NoRestart 2>$null | Out-Null
                ...
                Set-TestingModeStatus -Enable $true | Out-Null
                Write-Host ""
                Write-Host "[OK] Easy Setup completed successfully!" -ForegroundColor Green
                if ($st.SecureBoot) {
                    Write-Host "[i] Note: Secure Boot is enabled in BIOS (Optional for modern SUO)." -ForegroundColor Cyan
                }
                Write-Host ""
                $ans = Read-Host "Restart PC now to apply hypervisor changes? (Y/N) [Default: Y]"
            ...
            "X" { Revert-ToSafeMode -Mode 'CLI'; Start-Sleep -Seconds 2 }
